...
@implementation VCMVietCalendar (SolarLunar)
...
- (TimeSL) convertSunToLunar:(int)day month:(int)month year:(int)year timeZone:(int)timeZone
{
	...
	monthStart = [self getNewMoonDay:(int)k+1 timeZone:(double)timeZone];
	if (monthStart > dayNumber) {
		monthStart = [self getNewMoonDay:(int)k timeZone:(double)timeZone];
	}
	...
	lunarMonth = diff+11;
	if (b11 - a11 > 365) {
		CGFloat leapMonthDiff = [self getLeapMonthOffset:(int)a11 timeZone:(double)timeZone];
		if (diff >= leapMonthDiff) {
			lunarMonth = diff + 10;
			if (diff == leapMonthDiff) {
				lunarLeap = 1;
...
- (TimeSL) convertLunarToSun:(int)day month:(int)month year:(int)year lunarLeap:(int)lunarLeap timeZone:(int)timeZone
{
	...
	if (b11 - a11 > 365) {
		leapOff = [self getLeapMonthOffset:(int)a11 timeZone:(double)timeZone];
		leapMonth = leapOff - 2;
		if (leapMonth < 0) {
			leapMonth += 12;
		}
	...
	monthStart = (long)[self getNewMoonDay:(int)(k+off) timeZone:(double)timeZone];
