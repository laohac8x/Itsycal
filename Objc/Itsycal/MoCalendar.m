...
- (void)commonInitForMoCalendar
{
    ...
    for (MoCalCell *cell in _dowGrid.cells) {
			[cell setFont:[NSFont systemFontOfSize:SizePref.fontSize weight:NSFontWeightSemibold]];
    }
...
- (void)updateCalendar
{
    ...
    for (NSInteger col = 0; col < 7; col++) {
        ...
        [_dowGrid.cells[col] setString:dow];
        [_dowGrid.cells[col] setTextColor:[self columnIsMemberOfHighlightedDOWs:col]
